...
#+end_src
* Remember,our commits message, should mention:
** _viewGetUpdate() removed
** _on_updateUser_clicked() removed
* our CSV strategy for now:
** there are like a million ways to do insertion for parent-child rels
*** 1, we insert by name, challenging and more fun, definitely makes more sense, since users can check, and namings arent weird. but has a risk of complicating our code
*** 2, by number, this makes more logical sense, in addition it creates a "pivot" value, safer
*** 3, user manually sets this, the best method, because the system IS the communication. the user SEES the connection
* We need to move our enum class to its own simple header
#+begin_src c++
